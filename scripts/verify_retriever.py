...
async def run_sanity_check():
    ...
    if not cfg.tiger_database_url or "tsdbadmin" in cfg.tiger_database_url:
        ...
        print(
            "You must complete Task 1.2 (Tiger Cloud Setup) and update your .env before this will pass.\n"
        )

    if not cfg.google_api_key or cfg.google_api_key.startswith("<"):
        print("\n[!] WARNING: GOOGLE_API_KEY is missing or invalid.")
        print(
            "You must complete Task 1.3 and update your .env before this will pass.\n"
        )
    ...
    context = await retrieve_context_for_diff(
        diff=fake_diff, repo_full_name="async-ar15/mira-test-repo"
    )
    ...
    if test_content in context:
        print(
            "✅ SUCCESS: The retriever successfully found the inserted chunk using Tiger Cloud DiskANN!"
        )
    else:
