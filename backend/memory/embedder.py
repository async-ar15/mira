...
#
# MODEL CHOICE: text-embedding-004
...
#   For code semantics (function bodies, class signatures), 768 dims is sufficient.
#
...
#
# ASYNC CLIENT:
# google.generativeai SDK is used. All calls are async coroutines where possible,
# though embed_content may be synchronous depending on the SDK version; we wrap it if needed.
...
# Embedding vector dimensionality for text-embedding-004.
...
async def embed_text(text: str) -> list[float]:
    ...
    Raises:
        EmbeddingError: on any Google API failure. Caller handles gracefully.
    """
    ...
    try:
        # Note: Depending on the generativeai version, embed_content is synchronous.
        # It's fast enough that running it inline is acceptable for a single string.
        result = genai.embed_content(
            model=f"models/{cfg.google_embedding_model}",
            content=text,
        ...
        raise EmbeddingError(
            f"Google embedding call failed: {type(e).__name__}: {e}"
        ) from e
...
async def embed_batch(texts: list[str]) -> list[list[float]]:
    ...
    Raises:
        EmbeddingError: on any Google API failure.
    """
    if not texts:
        return []
    ...
    try:
        ...
        for position, embedding_vector in zip(indices, embeddings):
            results[position] = embedding_vector

        return results

    except Exception as e:
        raise EmbeddingError(
            f"Google batch embedding call failed: {type(e).__name__}: {e}"
        ) from e
