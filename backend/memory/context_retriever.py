...
#   It does not import embedder.py or tiger_client.py directly.
#   If we replace Tiger with another store, only this file and tiger_client.py change.
...
#     - Tiger unavailable  -> return ""  (search returns [] on error)
...
# Tiger's DiskANN always returns top_k results even if they are not very similar.
...
async def retrieve_context_for_diff(
    ...
    Queries Tiger Cloud (pgvectorscale DiskANN) for code chunks similar to
    this diff and returns them as a formatted context string for injection
    into agent prompts.
    ...
    # EmbeddingError means the Gemini embedding API is down or the key is wrong.
    ...
    # The score is NOT stored on the chunk — Tiger returns chunks ordered by
    # relevance but without an attached score field. We run a lighter filter:
    # exclude any chunk with empty content (defensive) and trust the ordering.
    # (RAG-Architecture.md: "If both agree, confidence rises; if neither agrees, skip.")
    relevant_results = [r for r in results if r.content and r.content.strip()]
...
def _format_context(results: list[CodeChunk]) -> str:
    """
    Formats Tiger search results (CodeChunk dataclasses) as a plain text context block.

    Each result becomes a labeled section with file path and code content.
    ...
    Every chunk is labeled with its source: file path AND chunk symbol (if available).
    Agents can include this provenance in their findings:
    "Similar pattern was seen in auth.py — check for same issue."
    """
    lines: list[str] = ["--- Relevant prior code context ---"]

    for chunk in results:
        file_path = chunk.path or "unknown"
        chunk_text = chunk.content or ""
        symbol = chunk.symbol  # optional function/class name from AST
