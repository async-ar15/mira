...
#   - Google API  (GOOGLE_API_KEY from environment)
...
#
# RUN LOCALLY:
#   GOOGLE_API_KEY=sk-... python3 -m pytest tests/eval_gate.py -v -s
#
...
# ---------------------------------------------------------------------------
# Skip guard — only run when GOOGLE_API_KEY is present.
#
...
pytestmark = pytest.mark.skipif(
    ...
    reason="GOOGLE_API_KEY not set — skipping eval gate (real LLM calls required)",
)
...
async def test_eval_gate_sample_pr_verdict():
    ...
    REAL:
      - All 4 agents call Google Gemini (GOOGLE_API_KEY from env)
...
+STRIPE_API_KEY = "sk_dummy_4242424242424242424242424242424242424242"
